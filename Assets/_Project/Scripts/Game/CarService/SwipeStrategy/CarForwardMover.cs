...
public class CarForwardMover : ICarMovementStrategy
{
    ...
    public CarForwardMover(Transform transform, BoxCollider self, int direction)
    {
    ...
    public event Action<CarForwardMover> ObstacleCollision;
    public event Action<CarForwardMover, CarRoad> CarRoadDetected;
    ...
    private bool IsCollision(float deltaDistance, Vector3 direction)
    {
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if(hit.collider.TryGetComponent(out CarRoad carRoad))
                CarRoadDetected?.Invoke(this, carRoad);
        }
